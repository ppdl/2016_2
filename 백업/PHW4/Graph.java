...
public class Graph
{
   ...
   public PriorityQueue<Edge> pqEdge = new PriorityQueue<Edge>();
    ...
    public HashMap<Node, Integer> dijkstra(Node source)
    {
    	...
    	PriorityQueue<Node> pq = new PriorityQueue<Node>(copyNodes);
    	...
    	while(!pq.isEmpty()){
    		...
    		copyNodes.remove(del);
    		Dijkstra.put(del, del.getValue());
    		pq = new PriorityQueue<Node>(copyNodes);
    	}
    ...
    public ArrayList<Edge> minSpanningTree()
    {
    	...
    	while(cloud.size() != numNodes)	{
    		pqEdge.clear();
    		for(Node rainDrop : cloud){
    			for(Edge e : NODE.get(rainDrop)){
    				pqEdge.offer(e);
    			}
    		}
    		
    		while(true){
    			Edge isNotCycle = pqEdge.poll();
    			if(cloud.contains(isNotCycle.getU()) && cloud.contains(isNotCycle.getV())) continue;
    			
    			if(cloud.contains(isNotCycle.getU())) cloud.add(isNotCycle.getV());
    			else cloud.add(isNotCycle.getU());
